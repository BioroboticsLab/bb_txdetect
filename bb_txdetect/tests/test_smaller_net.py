    ...
    def test_smaller_nets_output_size(self):
        ...
        for depth in [1, 3, 5]:
            with self.subTest(i=depth):
                ...
                net_classes = [tup[1] for tup
                               in inspect.getmembers(smaller_net,
                                                     inspect.isclass)]

                expected_output_size = torch.Size([testloader.batch_size, 2])
                known_failure_classes = [smaller_net.SmallerNet4,
                                         smaller_net.SmallerNet5]
                ...
                for i, net_class in enumerate(net_classes):
                    with self.subTest(i=i):
                        ...
                        if net_class not in known_failure_classes:
                            self.assertEqual(expected_output_size,
                                             outputs.shape)

...
from bb_txdetect.rotation import rotation_horizontal, crop_centered_bee_left_edge
from bb_txdetect.path_constants import INVERT, PAD
...
def _crop_and_rotate(metadata: [[dict]], padding: int,
                     input_image_folder: str, invert: bool):
    if not Path(input_image_folder).is_dir():
        raise Exception("input image folder not found: {}"
                        .format(input_image_folder))

    output_image_folder = "{}{}{}".format(str(Path(input_image_folder)),
                                          PAD, padding)
    ...
    for path in tqdm(sorted(glob("{}/*/*.png".format(input_image_folder)))):
        ...
        outimg = crop_centered_bee_left_edge(rotation_horizontal(img,
                                                                 *coordinates,
                                                                 invert=True),
                                             padding=16)

...
from bb_txdetect.load_data import Observation
...
def get_frame_plotter(obs: Observation, decode_n_frames: int,
                      crop_coordinates: [int]) -> FramePlotter:
...
class MetadataEntry(object):
    ...
    def csv_row(self):
        return reduce(lambda a, b: str(a) + "," + str(b),
                      [self.index, self.label, self.frame_id, self.offset_x,
                       self.offset_y, self.x1, self.y1, self.x2, self.y2,
...
def save_images(observations: [Observation], index: int,
    ...
    folder_label = "y" if any(o.trophallaxis_observed
                              for o in observations) else "n"
    ...
    if os.path.isdir(folder):
        raise Exception("folder path for saving images already exists: {}"
                        .format(folder))

    os.mkdir(folder)

    metadata = "index,label,frame_id,offset_x,offset_y,x1,y1,x2,y2," \
        ...
        while not done:
            try:
                ...
                print(str(datetime.datetime.now()),
                      "Exception in fp.get_image(), "
                      "will automatically retry in {} sec"
                      .format(sleep_time))
                sleep(sleep_time)

        imsave("{}/{:03}_{}.png".format(folder, i, obs.label), image)
        metadata += MetadataEntry(index=i, frame_id=obs.frame_id, xs=obs.xs,
                                  ys=obs.ys, orientations=obs.orientations,
                                  label=obs.label,
...
def rotate_image(image: np.ndarray, bee_orientation: float) -> np.ndarray:
    """rotates the image around the center to show one bee
    at the bottom looking north"""
